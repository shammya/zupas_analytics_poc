"""
migrate_deleted_orders.py

Reads deleted_order_flat.csv (output of fetch_deleted_to_csv.py) and inserts
rows into shipday_analytics.orders_flat with deleted_at, deleted_by,
deleted_by_role, and synced_at populated.

Usage:
    python migrate_deleted_orders.py            # run migration
    python migrate_deleted_orders.py --dry-run  # validate only, no insert
"""
...
CSV_PATH   = "/Users/shammo/Desktop/Work/Shipday/Scripts/Zupas_miggration/deleted_order_flat.csv"
BATCH_SIZE = 500

DELETED_BY_ROLE = "Dispatcher"   # default role — update if source data carries role info

# ── COLUMN DEFINITIONS ───────────────────────────────────────────────────────
# Extends CLICKHOUSE_COLUMNS from migrate_orders.py with the three deletion
# columns and synced_at so all data lands in one INSERT pass.
...
CLICKHOUSE_COLUMNS = [
    ...
    # Deletion columns (extra vs migrate_orders.py)
...
UINT32 = {
    ...
    "restaurant_id", "driver_vehicle_id",
}

# driver_id is Int32 (can be -1 sentinel from MySQL)
INT32 = {"driver_id"}
...
log = logging.getLogger("migrate_deleted")
...
def parse_row(row, synced_at: datetime):
    parsed = {}

    for col in CLICKHOUSE_COLUMNS:
        if col == "deleted_by_role":
            parsed[col] = DELETED_BY_ROLE
            continue
        if col == "synced_at":
            parsed[col] = synced_at
            continue
...
def main():
    ...
    for i, row_dict in enumerate(valid_rows):
        try:
            parsed_rows.append(parse_row(row_dict, synced_at))
        except Exception as e:
    ...
    pre_count = client.query(
        f"SELECT count() FROM {CLICKHOUSE_DATABASE}.{CLICKHOUSE_TABLE}"
    ).result_rows[0][0]
    log.info(f"Pre-insert CH count: {pre_count}")
    ...
    log.info(f"  CH delta    : {delta} (pre={pre_count}, post={post_count})")

    if delta != total_inserted:
        log.warning(
            f"  COUNT MISMATCH: expected {total_inserted}, got {delta}. "
            "Likely ReplacingMergeTree dedup on re-run — run OPTIMIZE TABLE to confirm."
        )
    else:
